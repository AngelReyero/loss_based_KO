...
def main(args):
    ...
    for s in args.seeds:
        ...
        importance_score=np.zeros((25, len(ns), p))
        true_importance=np.zeros(( len(ns), p))
        p_val=np.zeros((25, len(ns), p))
        executation_time = np.zeros((25, len(ns)))
    ...
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../results/csv/S-CPI_{y_method}_p{p}_cor{cor}_seed{s}.csv"))
    f_res.to_csv(
        csv_path,
        index=False,
        ) 
