...
#SBATCH --array=0-999%10  # 4 settings × 5 models × 50 seeds = 1000 jobs, 10 concurrent

# Define settings and models
settings=('single_index_threshold' 'cond_var' 'masked_corr' 'label_noise_gate')
...
python -m experiments.non_lin_p_val_ridge \
    --seed $seed \
    --setting $setting \
    --model $model
