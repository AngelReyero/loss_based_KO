...
from utils import GenNonLinDataset
...
def parse_args():
    ...
    parser.add_argument("--setting", type=str, choices=['single_index_threshold', 'mean_flip', 'cond_var', 'soft_interaction', 'masked_corr', 'piecewise_linear', 'label_noise_gate', 'sin_envelope'], required=True)
...
def get_base_model(model_name, random_state, n_jobs=1):
    if model_name == 'lasso':
        ...
        return GradientBoostingRegressor(n_estimators=200, max_depth=3, random_state=random_state, subsample=0.8)
    elif model_name == 'SL':
        estimators = [
...
def main(args):
    ...
    X_comp, y_comp, true_imp = GenNonLinDataset(n=n+n_r2, d=p, setting=setting, seed=seed)
    ...
    for idx, ptype in enumerate(['sign_test','wilcox']):
        start_time = time.time()
        p_val[25+idx, :] = cpi_knockoffs.score(X, y, n_perm=10, p_val=ptype)["pval"].reshape(p)
    ...
    # base name
    base = f"p_values_ridge_{setting}_{base_model_name}{n_tag}"
