...
def main(args):
    ...
    start_time = time.time()
    cpi_knockoffs_imputed= CPI_KO(
        estimator=model,
        imputation_model=clone(base_model),
        imputation_model_y=clone(base_model),
        random_state=seed,
        n_jobs=1)
    cpi_knockoffs_imputed.fit(X, y)
    imp_time_CPI_KO = time.time()-start_time
    r2_values[5:6] = r2_score(y_r2, model.predict(X_r2))

    start_time = time.time()
    cpi_KO_imputed_importance = cpi_knockoffs_imputed.score(X, y, n_perm=1,  p_val='wilcox')
    execution_time[6] = time.time() - start_time + tr_KO_time + imp_time_CPI_KO
    estim_imp[6, :p]= cpi_KO_imputed_importance["pval"].reshape((p,))

    execution_time[5] = execution_time[6]
    estim_imp[5, :p]= cpi_KO_imputed_importance["importance"].reshape((p,))
    ...
    for j in range(p):
        # Run the HRT
        sampler = RegressionSamplerSingle(
            estimator=clone(base_model), #RidgeCV(alphas=np.logspace(-3, 3, 10), cv=5),
            X_train=X_train,
            X_test=X_test,
            covariate_index=j
        )
        ...
        def conditional(X_eval=None):
            ...
            return sample, probs
        result = hrt(j,tstat_fn,X_train,X_test=X_test,conditional=conditional)
        estim_imp[7, j]= result['p_value']
    execution_time[7] = time.time() - start_time + tr_time
    r2_values[7] = r2_score(y_r2, model.predict(X_r2))
    ...
    methods = [
                ...
                'dCRT', 'HRT', "CPI_KO_imp", "CPI_KO_Wilcox_imp", "HRT_imp"
            ]
    f_res = pd.DataFrame()
    ...
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../results/res_csv/KO_imputed_{setting}_{base_model_name}_seed{seed}.csv"))
