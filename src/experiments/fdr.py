...
def main(args):
    ...
    for j in range(p):
        ...
        sampler = RegressionSamplerSingle(
            estimator=LassoCV(alphas=np.logspace(-3, 3, 10), cv=5, random_state=seed),
            X_train=X_train,
            X_test=X_test,
            covariate_index=j
        )
    ...
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../results/res_csv/KO_{setting}_{base_model_name}_seed{seed}.csv"))
