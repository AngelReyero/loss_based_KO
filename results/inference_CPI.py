...
csv_files = glob.glob(f"csv/S-CPI_hidimstats_p{p}_cor{cor}*.csv")
...
plt.savefig(f"figures/valid_inference_hidimstats_p{p}_cor{cor}_time_appendix.pdf", bbox_inches="tight")
...
cor=0.6
alpha=0.05
csv_files = glob.glob(f"csv/S-CPI_poly_p{p}_cor{cor}*.csv")
df = pd.concat((pd.read_csv(f) for f in csv_files), ignore_index=True)
...
plt.savefig(f"figures/valid_inference_poly_p{p}_cor{cor}_time_appendix.pdf", bbox_inches="tight")